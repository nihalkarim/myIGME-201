...
class Program
{
    ...
    start:
        ...
        for (nCntr = 0; nCntr < nQuestions; ++nCntr)
        {
            ...
            {
                dAnswer = val1 + val2;
                sQuestions = $"Enter your answer to 2 decimal places. Question #{nCntr + 1}: {val1} + {val2} => ";
            }
            else if (nOp == 1)
            {
                dAnswer = val1 - val2;
            ...
            // --> check for 2 digits. Find '.' in string  and make new  substring for after the period. Check length.
            do
            {
                ...
                {
                    Console.WriteLine("Please enter a number followed by 2 decimal points.");
                    bValid = false;
                }
            } while (!bValid);
